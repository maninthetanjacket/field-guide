...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      ...
      "Alternatively, block_id plus mode operates on an existing block instead of a new range: " +
      "mode 'restore' reactivates it with its summary byte-identical; " +
      "mode 'revise' replaces the summary of a decompressed block (reason required), keeps the same block_id, records the prior summary and reason in the block's history, and reactivates it.",
    inputSchema: {
      type: "object",
      properties: {
        session_id: {
        ...
        summary: {
          ...
          minLength: 1,
          description:
            "Summary text that will replace the anchor message's content in subsequent API requests. Author this from a landed place — it is what the model receives in place of the original messages. Required unless preview_only=true. With mode 'revise', the replacement summary for the existing block; not accepted with mode 'restore'.",
        },
        block_id: {
          type: "integer",
          minimum: 1,
          description:
            "An existing block to operate on instead of selecting a new range. Requires mode; cannot be combined with range selection, preview_only, confirm, or original_tokens. mode 'revise' requires the block to be decompressed; mode 'restore' on an already-active block changes nothing.",
        },
        mode: {
          type: "string",
          enum: ["restore", "revise"],
          description:
            "Required with block_id. 'restore' reactivates the block with its summary unchanged (byte-identical, cache-stable). 'revise' replaces the block's summary with the summary argument and reactivates it; the prior summary is kept in the block's summary history.",
        },
        reason: {
          type: "string",
          description:
            "Required with mode 'revise': what was wrong with the prior summary. Stored with the prior summary in the block's history and reported by list_compressions.",
        },
        focus: {
          type: "string",
          description:
  ...
  },
  {
    ...
    description:
      ...
      "The block remains in the registry as inactive. compress with this block_id and mode 'restore' reactivates it unchanged; mode 'revise' replaces its summary.",
    inputSchema: {
      type: "object",
      properties: {
        session_id: {
  ...
  },
  {
    ...
    description:
      ...
      "Each block's metadata includes its revision count and the reason and time of its most recent revision.",
    inputSchema: {
      type: "object",
      properties: {
        session_id: {
...
export async function handleCompress(args: unknown): Promise<ToolResult> {
  ...
  try {
    ...
    targetBlockId = optionalPositiveInt(a["block_id"], "block_id");
    mode = optionalString(a["mode"], "mode");
    reason = optionalString(a["reason"], "reason");
  ...
  // Operations on an existing block take their own path, checked before range
  // validation so that errors name the actual conflict.
  if (targetBlockId !== undefined || mode !== undefined) {
    return handleExistingBlock(sessionId, a, { blockId: targetBlockId, mode, summary, reason, focus, summaryTokens });
  }
  if (reason !== undefined) {
    return errorResult("reason is only accepted with block_id and mode 'revise'");
  }
...
/** compress arguments that select or preview a new range; meaningless with block_id. */
const RANGE_ONLY_ARGS = [
  "compressed_uuids",
  "start_turn",
  "end_turn",
  "first_phrase",
...
interface ExistingBlockOptions {
...
/**
 * compress with block_id: restore a decompressed block unchanged, or revise
 * its summary. Replaces the former `recompress` tool, which accepted and
 * silently ignored a summary argument (returning success with the old text
 * restored). Both modes are explicit and reject arguments they would not use.
 */
async function handleExistingBlock(
  sessionId: string,
  a: Record<string, unknown>,
  opts: ExistingBlockOptions,
): Promise<ToolResult> {
  const { blockId, mode, summary, reason, focus, summaryTokens } = opts;

  if (blockId === undefined) {
    return errorResult("mode requires block_id: it selects which existing block to restore or revise");
  }
  if (mode === undefined) {
    return errorResult(
      `block_id ${blockId} names an existing block, so mode is required: ` +
        "'restore' reactivates it with its summary unchanged; 'revise' replaces its summary (pass summary and reason).",
    );
  }
  if (mode !== "restore" && mode !== "revise") {
    return errorResult(`mode must be 'restore' or 'revise' (got '${mode}')`);
  }
  const conflicting = RANGE_ONLY_ARGS.filter((k) => a[k] !== undefined);
  if (conflicting.length > 0) {
    return errorResult(
      `block_id cannot be combined with ${conflicting.join(", ")}: ` +
        "those select or preview a new range, while block_id targets an existing block",
    );
  }

  const block = await readBlock(sessionId, blockId);
  if (block === null) {
    return errorResult(`Block ${blockId} not found in session ${sessionId}`);
  }

  if (mode === "restore") {
    const extras = (["summary", "reason", "focus", "summary_tokens"] as const).filter(
      (k) => a[k] !== undefined,
    );
    if (extras.length > 0) {
      return errorResult(
        `mode 'restore' reactivates block ${blockId} with its summary unchanged and does not accept ${extras.join(", ")}. ` +
          "To change the summary, use mode 'revise' with summary and reason.",
      );
    }
    const alreadyActive = block.active;
    if (!alreadyActive) {
      block.active = true;
      await writeBlock(sessionId, block);
    }
  ...
  if (block.active) {
    return errorResult(
      `Block ${blockId} is active. Decompress it first, so its original turns are restored ` +
        "and can be re-read, then revise it from those turns rather than from the current summary.",
    );
  }
  if (summary === undefined || summary.trim().length === 0) {
    return errorResult("mode 'revise' requires summary: the replacement text for the block");
  }
  if (reason === undefined || reason.trim().length === 0) {
    return errorResult("mode 'revise' requires reason: what was wrong with the prior summary");
  }
  if (summary === block.summary) {
    return errorResult(
      `summary is identical to block ${blockId}'s current summary; use mode 'restore' to reactivate it unchanged`,
    );
  }

  block.summary_history = [
    ...block.summary_history,
    { summary: block.summary, replaced_at: new Date().toISOString(), reason },
  ];
  block.summary = summary;
  block.summary_tokens = summaryTokens ?? countTextTokens(summary);
  if (focus !== undefined) block.focus = focus;
  block.active = true;
  await writeBlock(sessionId, block);
  ...
  return successResult({
    ...
    mode: "revise",
    active: true,
    revision_count: block.summary_history.length,
    summary_tokens: block.summary_tokens,
    messages_replaced: block.compressed_uuids.length,
  });
}
...
/**
 * Reject arguments a tool's schema does not declare. Without this, a stray
 * argument is silently dropped and the call reports success, so the caller
 * believes it did something it did not.
 */
function unknownArgsError(name: string, args: unknown): string | null {
  const tool = TOOLS.find((t) => t.name === name);
  if (tool === undefined || typeof args !== "object" || args === null) return null;
...
export async function dispatch(
  ...
  const argError = unknownArgsError(name, args);
  if (argError !== null) return errorResult(argError);
  ...
  switch (name) {
    ...
    case "recompress":
      // Removed tool; answer callers working from older docs with the replacement.
      return errorResult(
        "recompress has been folded into compress: pass block_id with mode 'restore' to reactivate " +
          "a decompressed block unchanged, or mode 'revise' with summary and reason to replace its summary.",
      );
