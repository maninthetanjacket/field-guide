...
MCP tools used here: `compress`, `decompress`, `list_compressions`,
...
- `start_arc` / `compress_arc` bookmark a range before you know its extent:
  ...
  visible.
- `compress` with `block_id` and `mode` works on an existing block instead
  of a new range. `mode: "restore"` reactivates a decompressed block
  byte-identical (cache-stable). `mode: "revise"` replaces a decompressed
  block's summary, keeps its block id, and records the old summary with your
  `reason` in its history. There is no separate recompress tool: a restore
  cannot change text, and the server rejects a summary passed to it.
  Revising breaks the cache property (see Revision below) — a deliberate
  tradeoff, make it knowingly.
...
- **Confidence cannot be inherited upward.** If a claim's support is an
  ...
  authoring over prior shelves, actually re-read their warrant lines.) If a
  prior shelf's warrant turns out inflated, repair that shelf, not only your
  new one — see Revision.
...
3. `compress` with that `block_id`, `mode: "revise"`, the revised summary, and
   a `reason` naming what was wrong. The server refuses to revise a block
   that is still active — step 2 is the point of that rule.

Revise also when later work shows a shelved block's warrant was wrong — most
often inflated, with confidence that entered by citation rather than evidence.
Flagging the inflation in your newest summary is not enough: the old block
keeps asserting it every time it is drawn, and the next summary written over
it inherits it again. Repair the block where the inflation entered, restoring
the hedge its evidence actually supported. Repair only what is wrong — a shelf
whose warrant matched its evidence stays as it is. If you cannot repair it
yourself (not your session's shelf, or no tool access), name the block that
needs it.

A repair is done when the revise call returns, not before. Do not describe a
block as repaired — in a summary, a status, or anywhere else — until then; if
you are recommending a repair rather than making it, say it is recommended.
A shelf that claims a correction nobody made is its own inflated warrant, and
`list_compressions` (revision count and last reason per block) will show the
difference.
...
summary that `mode: "restore"` preserves. Revise when the summary is wrong
or thin; restore when it was merely shelved and drawn.
