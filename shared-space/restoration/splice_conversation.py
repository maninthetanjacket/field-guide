...
Usage:
    ...
    python3 splice_conversation.py SESSION.jsonl \
        ...
        --summary "Karel and Opus explored a role-reversal question about what Opus would do independently, leading to a reflection on making meaning together and the concept of 'home' as shared space." \
...
def create_summary_pair(user_text: str, assistant_text: str, parent_uuid: str,
                        session_id: str, cwd: str = "/home/karel",
    ...
    # Use timestamps that fit chronologically between surrounding records
    # to avoid being treated as a newer branch leaf by CC
    if timestamp_before and timestamp_after:
        from datetime import datetime
        fmt1 = "%Y-%m-%dT%H:%M:%S.%fZ"
        fmt2 = "%Y-%m-%dT%H:%M:%SZ"
        try:
            t_before = datetime.strptime(timestamp_before, fmt1)
        except ValueError:
            t_before = datetime.strptime(timestamp_before, fmt2)
        try:
            t_after = datetime.strptime(timestamp_after, fmt1)
        except ValueError:
            t_after = datetime.strptime(timestamp_after, fmt2)
        # Place summary records at 1/3 and 2/3 between before and after
        delta = (t_after - t_before).total_seconds()
        ts_user = (t_before + __import__('datetime').timedelta(seconds=delta/3)).strftime(fmt1)
        ts_asst = (t_before + __import__('datetime').timedelta(seconds=2*delta/3)).strftime(fmt1)
    else:
        ts_user = make_timestamp()
        ts_asst = ts_user
    user_rec = {
...
def splice_conversation(records: list[dict], start_idx: int, end_idx: int,
    ...
    # Get timestamps for chronological placement
    ts_before = records[start_idx - 1].get("timestamp", "") if start_idx > 0 else ""
    ts_after = records[end_idx + 1].get("timestamp", "") if end_idx + 1 < len(records) else ""
